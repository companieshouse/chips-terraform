...
resource "aws_ssm_association" "ansible_check" {
  association_name = "${var.application}-ansible-check"

  targets {
    key    = "InstanceIds"
    values = aws_instance.reginit_ec2.*.id
  }
  ...
  output_location {
    ...
    s3_key_prefix  = "${var.application}/ansible-check/"
...
resource "aws_ssm_association" "ansible_apply" {
  ...
  association_name = "${var.application}-ansible-apply"
  ...
  output_location {
    s3_bucket_name = local.ssm_data.ssm_logs_bucket_name
    s3_key_prefix  = "${var.application}/ansible-apply/"
    s3_region      = var.aws_region
  }
}
...
resource "aws_ssm_maintenance_window" "maintenance_window" {
  name     = "${var.application}-maintenance-window"
...
resource "aws_ssm_maintenance_window_target" "target" {
  ...
  name          = "${var.application}-maintenance-window-target"
